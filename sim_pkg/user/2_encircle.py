...
# # -*- coding: utf-8 -*-
# # Two-Ring Encircle: Velocity-based spacing using P2P
# # Inner ring CCW, outer ring CW. Bots adjust speed based on distance
# # to neighbors on their ring to achieve equal spacing.
# # Python 2.7 + Coachbot messaging + lab logging pattern
...
# # ----------------- Two-ring parameters -----------------
# # Inner and outer ring radii (tune these for the aesthetic you want)
# R_INNER = SAFE_BUBBLE + 0.22
# R_OUTER = SAFE_BUBBLE + 0.40

# # Threshold to decide which ring a bot belongs to
# R_SPLIT = 0.5 * (R_INNER + R_OUTER)

# # Tolerance to treat a neighbor as "on my ring" for spacing
# R_RING_TOL = 0.10
...
# K_R, RADIAL_CLAMP = 1.3, 0.12
# MIN_LINEAR_SEP = 0.18
# # IDEAL_ANGULAR_SEP kept as reference, but per-ring we estimate from neighbors
# IDEAL_ANGULAR_SEP = 2 * math.pi / 8.0
...
#     rid = get_id(robot)
#     logw("Bot starting with ID: %d (two-ring encircle)" % rid)
...
#         # --- decide ring based on which radius I'm closer to ---
#         dist_to_inner = abs(r - R_INNER)
#         dist_to_outer = abs(r - R_OUTER)

#         if dist_to_inner <= dist_to_outer:
#             # closer to inner ring → move CCW
#             R_TARGET_LOCAL   = R_INNER
#             DIRECTION_LOCAL  = +1   # CCW
#         else:
#             # closer to outer ring → move CW
#             R_TARGET_LOCAL   = R_OUTER
#             DIRECTION_LOCAL  = -1   # CW
...
#         if now - last_logw > logw_PERIOD:
#             re = R_TARGET_LOCAL - r
#             logw("[velocity_spacing_2rings] id=%d neighbors=%d radius_error=%.3f r=%.3f ring_target=%.3f"
#                  % (rid, len(nbrs), re, r, R_TARGET_LOCAL))
#             last_logw = now
...
def usr(robot):
    ...
    while True:
        ...
        now = robot.get_clock()
        # Log position data
        logw("POSE vid=%d t=%.3f x=%.3f y=%.3f th=%.3f" %
            (rid, now, x, y, th))
        ...
        # This doesn't change during the experiment!
