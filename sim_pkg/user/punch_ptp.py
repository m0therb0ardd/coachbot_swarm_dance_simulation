# # -*- coding: utf-8 -*-
# # PUNCH (SIM): heavy • sudden • direct
# # Staccato bursts: quick aim → hard surge to +x̂ → full stop; strong separation.
...
# FWD_PUNCH   = 1.00
# DT_MS       = 40
# CMD_SMOOTH  = 0.12      # snappier than FLOAT
# VEL_SLEW    = 18        # allow big jumps (heavy/sudden)

# # --- punch timing ---
# AIM_TIME    = 0.25
# BURST_TIME  = 0.55
# HOLD_TIME   = 0.10      # short hold after burst (weight/impact)
# COOLDOWN    = 1.10
# RIGHT_HEADING = 0.0
...
# SEP_R       = 0.28
# NEIGH_R     = 0.75
# K_SEP       = 0.40       # stronger than FLOAT
# K_COH       = 0.08
# K_ALI       = 0.10
...
# def soft_obstacle_force(x, y, maxf=0.75, w=0.12):
...
#     lastL = lastR = 0
#     state = "idle"
#     aim_until = burst_until = hold_until = 0.0
...
#         else:
#             if state=="burst": robot.set_led(100,0,70)   # magenta attack
#             elif state=="aim": robot.set_led(100,100,0)  # yellow prep
#             elif state=="hold": robot.set_led(80,0,0)    # heavy hold
...
#         # --- heavy separation & safety fields ---
...
#         # emergency evade if someone is *very* close
#         evade_h = None
...
#         if state == "idle":
#             if now >= next_time:
#                 state = "aim"; aim_until = now + AIM_TIME
#                 target_h = RIGHT_HEADING   # direct punch → right
#         elif state == "aim":
#             if now >= aim_until:
#                 state = "burst"; burst_until = now + BURST_TIME
#             # if too close while aiming, adjust target to avoid collision
#             if evade_h is not None:
#                 target_h = 0.7*target_h + 0.3*evade_h
#         elif state == "burst":
#             if now >= burst_until:
#                 state = "hold"; hold_until = now + HOLD_TIME
#         elif state == "hold":
#             if now >= hold_until:
#                 state = "idle"; next_time = now + COOLDOWN

#         # --- control mapping ---
#         if state == "aim":
#             # quick turn toward target, small forward bias
#             err = wrap(target_h - th)
#             fwd = FWD_AIM
#         elif state == "burst":
#             # heavy, direct surge to +x̂ with safety field blended
#             vx = math.cos(RIGHT_HEADING) + repx + ex + ox + alx
#             vy = math.sin(RIGHT_HEADING) + repy + ey + oy + aly
...
#             robot.set_vel(0,0); robot.delay(DT_MS); continue
#         else:
#             # idle: very small drift, stay responsive
#             vx = ex+ox+repx+alx
#             vy = ey+oy+repy+aly
...
#         # allow **sudden** changes (heavier feel): large slew caps
...
# -*- coding: utf-8 -*-
# PUNCH (SIM): aggressive • synchronized • direct
# Shockwave pulses -> AIM -> CHARGE -> IMPACT (double tap) -> RECOIL -> RESET
# P2P token sync, feint if blocked, hard accel/brake, strong separation/obstacle/walls.
