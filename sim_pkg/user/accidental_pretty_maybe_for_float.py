...
def usr(robot):
    ...
    # compute each robot's radius from center
    radii = []
    ...
    # compute *my* initial radius and angle, and ring label
    ...
    # ring classification (just a label, mostly for your own reference)
    if r0 <= r_thresh:
        my_ring = 0   # inner
    else:
        my_ring = 1   # outer
        ...
        while True:
            ...
            if msgs:
                ...
                for m in msgs:
                    if not m:
                        continue
                    # ignore gossip-like short messages
                    if len(m) < HB_BYTES:
                        continue
                    try:
